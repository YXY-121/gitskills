...
public class UserController  extends BaseController{
    ...
    public CommonReturnType register(@RequestParam(name="telphone")String telphone,
        ...
        //验证手机号和对应的otpcode相符合
        //todo 改为用redis存token
        String inRedisOtpCode = (String)redisTemplate.opsForValue().get("phone_"+telphone);
    ...
    //用户获取otp短信接口
    @RequestMapping(value = "/getCode",method = {RequestMethod.POST},consumes={CONTENT_TYPE_FORMED})
    @ResponseBody
    public CommonReturnType getOtp(@RequestParam(name="telphone")String telphone){
      ...
      //  httpServletRequest.getSession().setAttribute(telphone,otpCode);
            redisTemplate.opsForValue().set("phone_"+telphone,otpCode);
            redisTemplate.expire("phone_"+telphone,5,TimeUnit.MINUTES);
        //将OTP验证码通过短信通道发送给用户,省略了发送到用户手机的步骤
    ...
    public CommonReturnType getUser(@RequestParam(name="id") Integer id) throws BusinessException {
        ...
        UserModel userModel = userService.getUserByCache(id);
    ...
    public CommonReturnType login(@RequestParam(name="telphone")String telphone,
        ...
        redisTemplate.opsForValue().set("user_"+token,userModel);
